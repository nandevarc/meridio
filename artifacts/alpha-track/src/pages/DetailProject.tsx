...
import {
  STATUS_STYLES, PRIORITY_COLORS, PLAY_STATUS_COLORS,
  ...
  return (
    <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "12px 16px" }}>
        ...
        return (
          <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 6, paddingBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 6, marginLeft: 12, marginRight: 12 }}>
              {[1, 2, 3, 4, 5].map((n) => (
                <div key={n} style={{ width: 12, height: 12, borderRadius: "50%", background: n <= val ? ACCENT : BORDER_SUB, border: n <= val ? "none" : `1px solid ${BORDER}`, flexShrink: 0 }} />
              ))}
            </div>
            <span style={{ fontSize: 14, fontWeight: 600, color: TEXT_PRI, width: 20, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{val}</span>
          </div>
        );
      })}
      <div style={{ borderTop: `2px solid ${BORDER}`, paddingTop: 12, marginTop: 4, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: ACCENT, fontVariantNumeric: "tabular-nums" }}>{total} / 25</span>
      </div>
    </div>
  );
}
...
export default function DetailProject({ id }: Props) {
  ...
  function handleDelete() {
    libDeleteProject(project!.id);
    showToast("project dihapus");
    setLocation("/");
  }

  function handleVerdictChange(v: Verdict | null) {
    if (!project) return;
  ...
  function handleMarkReviewed() {
    if (!project) return;
    markAsReviewed(project.id);
    const updated = { ...project, lastReviewedAt: new Date().toISOString() };
    setProject(updated);
  ...
  const links = [
    ...
    { key: "twitter",  label: "Twitter / X", url: project.links.twitter  },
    { key: "discord",  label: "Discord",     url: project.links.discord  },
    { key: "telegram", label: "Telegram",    url: project.links.telegram },
    ...
    <div style={{ background: "#FAFAFA", minHeight: "100vh", paddingBottom: 32 }}>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16, alignItems: "center" }}>
          ...
          {project.conviction && <span style={pillStyle}>{project.conviction}</span>}
        </div>
            ...
            {verdictOptions.map((v) => {
              ...
              return (
                <button key={v} type="button" onClick={() => handleVerdictChange(isSelected ? null : v)} data-testid={`verdict-quick-${v}`}
                  style={{ flex: 1, height: 36, borderRadius: 8, border: `1px solid ${isSelected ? s.border : BORDER}`, background: isSelected ? s.bg : SURF_RAISED, color: isSelected ? s.text : TEXT_MUTED, fontSize: 11, cursor: "pointer", fontWeight: isSelected ? 600 : 400, transition: "all 150ms ease" }}>
                  {v}
                </button>
              );
            })}
        ...
        {/* Analysis */}
        {(project.description || project.narrative || project.builders.length > 0 || project.ct.names.length > 0 || project.ct.count > 0 || project.decisionNote || project.biasCheck) && (
          <>
            ...
            {project.builders.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ color: TEXT_MUTED, fontSize: 10, marginBottom: 6, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em" }}>Builder & Team</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {project.builders.map((b) => (
                    <span key={b} style={{ background: SURF_RAISED, color: TEXT_SEC, border: `1px solid ${BORDER}`, borderRadius: 4, padding: "2px 8px", fontSize: 11, fontWeight: 500 }}>{b}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {project.biasCheck && (
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ color: TEXT_SEC, fontSize: 14, lineHeight: 1.6 }}>{project.biasCheck}</div>
              </div>
            )}
          </>
        )}
        ...
        {/* Play */}
        {(project.playType.length > 0 || project.actionRequired || project.playNotes || project.playStatus !== "Belum Ada") && (
          <>

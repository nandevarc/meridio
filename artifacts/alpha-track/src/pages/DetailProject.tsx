...
import { Project, STATUS_STYLES, PRIORITY_COLORS, PLAY_STATUS_COLORS } from "../types";
import { getProject, deleteProject, formatDate, formatDateTime } from "../utils/storage";
    ...
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
      {tags.map((t) => (
        <span key={t} style={{
          ...
          border: "1px solid var(--border)", borderRadius: 999, padding: "3px 10px", fontSize: 11, ...style
        }}>
          {t}
        </span>
      ))}
...
function SectionLabel({ title }: { title: string }) {
  return (
    <div style={{
      color: "var(--text-muted)", fontSize: 11, marginTop: 20, marginBottom: 12,
    ...
    <div style={{ background: "var(--bg-base)", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          <span style={{
            background: "var(--bg-elevated)", color: priorityColor, border: "1px solid var(--border)",
            borderRadius: 999, padding: "4px 12px", fontSize: 12,
          }}>
            {project.priority}
          </span>
        ...
        {/* Analysis */}
        {(project.description || project.narrative || project.builder || project.ctSignal || project.quickScore != null || project.decisionNote) && (
          <>
            ...
            <Row label="CT Signal" value={project.ctSignal} />
            {project.quickScore != null && <Row label="Quick Score" value={`${project.quickScore}/25`} />}
          ...
          <>
            ...
            {project.playStatus !== "Belum Ada" && (
              <div style={{ marginBottom: 10 }}>
                ...
                <span style={{
                  color: PLAY_STATUS_COLORS[project.playStatus],
                  fontSize: 13,
                }}>
                  {project.playStatus}
                </span>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {project.playTypes.map((pt) => (
                    <span key={pt} style={{ background: "var(--emerald-dim)", color: "var(--emerald)", borderRadius: 999, padding: "3px 10px", fontSize: 11 }}>
                      {pt}
                    </span>
                  ))}
                </div>
        ...
        <button
          ...
          data-testid="btn-generate-share"
          style={{
            width: "100%",
            marginTop: 20,
            background: "var(--bg-elevated)",
        ...
        <>
          ...
          >
            ...
            <button
              type="button"
              onClick={() => {
                navigator.clipboard?.writeText(generateShareText()).catch(() => {});
                showToast("teks disalin");
                setShowShare(false);
              }}
              data-testid="btn-salin"
              style={{
                width: "100%",
                height: 48,
                background: "var(--red)",
            ...
            <div style={{ textAlign: "center" }}>
              <button
                type="button"
                onClick={() => setShowShare(false)}
                style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: 12, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace" }}
              >
                Tutup
              </button>
            </div>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                type="button"
                onClick={() => setShowDeleteModal(false)}
                data-testid="btn-batal-delete"
                style={{
                  flex: 1, height: 44, background: "var(--bg-elevated)", color: "var(--text-secondary)",
              ...
              <button
                ...
                data-testid="btn-confirm-hapus"
                style={{
                  flex: 1, height: 44, background: "var(--red-dim)", color: "var(--red)",
                  border: "1px solid #991b1b", borderRadius: 10, fontSize: 13, cursor: "pointer",
                  fontFamily: "'IBM Plex Mono', monospace",
                }}
              >
                Hapus
